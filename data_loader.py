...
class Dataset:
    ...
    def read(self):
        ...
        for file_name in ["train.txt", "valid.txt", "test.txt"]:
            with open(
                os.path.join(self.config.dataset_path, file_name),
                "r",
                encoding="utf-8",
            ) as f:
    ...
    def load_error_triplets(self):
        ...
        if not os.path.isfile(err_path):
            logging.warning(
                "error triple file not found at %s, using zero injected errors",
                err_path,
            )
            return []
    ...
    def read_rules(self):
        ...
        if not os.path.isfile(rules_path):
            logging.warning(
                "rules file not found at %s, using empty rules for all relations",
                rules_path,
            )
            self.rel2rules = {self.relation[i]: {} for i in range(self.relation.cnt)}
        else:
